...
watch(
  ...
  (pos, oldPos) => {
    ...
    if (pos) {
      guessMarker.value = new maplibregl.Marker({
        element: createMarkerElement("guess"),
      })
...
watch(
  ...
  (show) => {
    ...
    if (show && props.answerPosition && props.guessPosition) {
      answerMarker.value = new maplibregl.Marker({
        element: createMarkerElement("answer"),
      })
...
<template>
  <div ref="mapContainer" class="w-full h-full cursor-crosshair" />
</template>
