...
export const getScoreMessage = (
  ...
  if (score >= 900)
    return { text: "Parfait !", emoji: "🎯", color: "text-emerald-400" };
  if (score >= 750)
  ...
  if (score >= 300)
    return { text: "Correct", emoji: "👌", color: "text-yellow-400" };
  if (score >= 150)
    return { text: "Passable", emoji: "😅", color: "text-orange-400" };
  if (score > 0)
    return { text: "Presque...", emoji: "😬", color: "text-orange-500" };
