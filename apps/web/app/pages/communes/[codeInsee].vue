...
const quizAddresses = computed(() => {
  ...
  return commune.value.voies.flatMap((voie) =>
    (voie.numeros ?? [])
...
useSeoMeta({
  title: () =>
    commune.value ? `Quiz ${commune.value.nom} - GeoQuiz` : "Chargement...",
  description: () =>
    commune.value
    ...
    <div v-else>
      <div class="mb-8 text-center">
        ...
        <p class="text-slate-500 mt-1">
          {{ commune.codePostal }} · {{ commune.voieCount }} voies ·
          {{ quizAddresses.length }} adresses
        </p>
      </div>

      <QuizGame :addresses="quizAddresses" :commune-name="commune.nom" />
    </div>
  </UContainer>
</template>
